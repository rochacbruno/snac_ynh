...
- **Multiple instances are supported** on different domains (e.g. `domain1.tld` and `domain2.tld`). However, only one snac2 instance can be installed per domain, because ActivityPub federation endpoints (WebFinger, NodeInfo, Mastodon API) must be handled by a single instance per domain.
